...
let dcTimeIndex = 3;
let dcCurrentDay = 2;
let dcTimeColumn = 'time_of_day';
let dcAmbientColumn = 'ambient_event';
...
function advanceAtmosphere() {
  try {
    ...
    db.prepare(`UPDATE city_atmosphere SET weather=?, ${dcTimeColumn}=?, ${dcAmbientColumn}=?, moon_phase=?, day=?, updated_at=? WHERE id=1`).run(
...
(() => {
  try {
    ...
    if (r) {
      ...
      if (cols.includes('timeOfDay')) dcTimeColumn = 'timeOfDay';
      else if (cols.includes('timeofday')) dcTimeColumn = 'timeofday';
      if (cols.includes('ambientEvent')) dcAmbientColumn = 'ambientEvent';
