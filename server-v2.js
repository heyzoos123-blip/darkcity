...
app.post("/api/auth/signup", authLimiter, async (req, res) => {
  try {
    ...
    await addChronicle("founding", `New human observer joined: ${displayName || "Anonymous"}`, null, [], null, 1);
    res.status(201).json({ success: true, message: "Account created. Welcome to darkcity.wtf.", humanId: result.rows[0].id });
